-- Redesign Phase 2: category color needs the field group, not just the
-- display name. "Bourbon", "Rye" and "Scotch" are all `whiskey` for color
-- purposes, and the view had no column that said so.
--
-- A replace cannot reorder or insert columns, only append, so the view is
-- dropped and rebuilt, same as 0003 and 0004.
